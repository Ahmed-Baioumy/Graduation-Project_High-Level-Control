...
if __name__ == '__main__':
    ...
    # Loads the weights
    trained_model.load_weights("RAW_EMG_DATA/CNN_model_checkpoint/cp.ckpt")
    ...
    def predict():
    	...
    	for i in range (len(EMG_DATA)):
    		Channel_1.append(EMG_DATA[i][0])
    		Channel_2.append(EMG_DATA[i][1])
    		Channel_3.append(EMG_DATA[i][2])
    		Channel_4.append(EMG_DATA[i][3])
    		Channel_5.append(EMG_DATA[i][4])
    		Channel_6.append(EMG_DATA[i][5])
    		Channel_7.append(EMG_DATA[i][6])
    		Channel_8.append(EMG_DATA[i][7])
    	...
    	Frame_as_image = np.array(frame_as_image).reshape(-1, 8, 8, 8)
    	
    	Frame_as_image = tf.keras.utils.normalize(Frame_as_image,axis=1)
    	
    	y = np.argmax(trained_model.predict(Frame_as_image))
    	
    	for classes, num_of_classes in Classes_Map.items():
    		if num_of_classes == y:
    			print(classes)
